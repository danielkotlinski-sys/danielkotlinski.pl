...
//   website-content-crawler × 4 brands:  ~$0.36  (4 × ~0.3 CU × $0.30)
...
// Jina (fallback only, rarely used):    ~$0.00
//
// TOTAL ESTIMATE:                       ~$3.86 per scan
// With SCAN_MODE=test (Haiku):          ~$2.50 per scan
// ─────────────────────────────────────────────────────────────────
...
const PRICING = {
  ...
  // Jina — fallback only (used when Apify website-content-crawler is unavailable)
  'jina-reader': { perRequest: 0.01 },
  'jina-screenshot': { perRequest: 0.01 },
  // Apify — Bronze plan: $0.30/CU
  // Used for website + social media scraping + ads.
  'apify-website': { perRun: 0.09 },      // ~0.3 CU × $0.30 (playwright:firefox, 4 pages)
